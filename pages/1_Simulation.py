from des_classes import g, Patient, Model, Trial, Caller
...
with st.sidebar:
    patient_inter = st.slider("patient_inter",
    min_value = 1, max_value = 10, value = 3, step=1)
    
    call_inter = st.slider("call_inter",
    min_value = 1, max_value = 50, value = 10, step=1)

    mean_reg_time = st.slider("mean_reg_time",
    min_value = 1, max_value = 10, value = 2, step=1)

    mean_gp_time = st.slider("mean_gp_time",
    min_value = 1, max_value = 10, value = 8, step=1)
    
    mean_book_test_time = st.slider("mean_book_test_time",
    min_value = 1, max_value = 10, value = 4, step=1)

    mean_call_time = st.slider("mean_call_time",
    min_value = 1, max_value = 10, value = 4, step=1)

    number_of_receptionists = st.slider("number_of_receptionists",
    min_value = 1, max_value = 10, value = 1, step=1)
    
    number_of_gps = st.slider("number_of_gps",
    min_value = 1, max_value = 10, value = 2, step=1)

    prob_book_test = st.slider("prob_book_test",
    min_value = 0.00, max_value = 1.00, value = 0.25, step=0.01)

    sim_duration = st.slider("sim_duration",
    min_value = 10, max_value = 1000, value = 480, step=1)

    number_of_runs = st.slider("number_of_runs",
...
if go_button:
            st.write(f"patient_inter: {patient_inter}")
            st.write(f"call_inter: {call_inter}")
            st.write(f"mean_reg_time: {mean_reg_time}")
            st.write(f"mean_gp_time: {mean_gp_time}")
            st.write(f"mean_book_test_time: {mean_book_test_time}")
            st.write(f"mean_call_time: {mean_call_time}")
            st.write(f"number_of_receptionists: {number_of_receptionists}")
            st.write(f"number_of_gps: {number_of_gps}")
            st.write(f"prob_book_test: {prob_book_test}")
            st.write(f"sim_duration: {sim_duration}")
            st.write(f"number_of_runs: {number_of_runs}")

            #st.write(f"test: {test}")

            # Inter-arrival times
            g.patient_inter = patient_inter
            g.call_inter = call_inter

            # Activity times
            g.mean_reg_time = mean_reg_time
            g.mean_gp_time = mean_gp_time
            g.mean_book_test_time = mean_book_test_time
            g.mean_call_time = mean_call_time

            # Resource numbers
            g.number_of_receptionists = number_of_receptionists
            g.number_of_gps = number_of_gps

            # Branch probabilities
            g.prob_book_test = prob_book_test
            ...
            st.write(caller_results.sample(25))

            st.write(patient_results.sample(25))

            st.write("=== Receptionist Stats ===")
